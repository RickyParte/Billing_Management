...
class stockClass:
    def __init__(self,root):
        ...
        blank_space =" "
        self.root.title(110*blank_space+"BVCOENM Billing and Management System |  Developed by SE Student")
    ...
    def add(self):
        ...
        try:
            if self.var_sup.get()=="Select" or self.var_name.get()=="":
                ...
                cur.execute("Select * from stock where itemname=? & pid=?",(self.var_name.get(),self.var_pid.get(),))
                row=cur.fetchone()
                if row!=None:
                    messagebox.showerror("Error","Product/PID Already Present, try different",parent=self.root)
                else:
